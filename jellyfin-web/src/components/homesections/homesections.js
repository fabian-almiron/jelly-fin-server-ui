...
import { loadHeroSlider, pauseHeroSlider, resumeHeroSlider, destroyHeroSlider } from 'components/heroSlider/heroSlider';
...
export function loadSections(elem, apiClient, user, userSettings) {
    ...
    return queryClient
        ...
        .then(function (userViews) {
            ...
            if (userViews.length) {
                // ── Hero slider ───────────────────────────────────────────────
                // Mount a wrapper as a sibling immediately before the sections div
                let heroWrapper = elem.parentNode.querySelector('.heroSliderWrapper');
                if (!heroWrapper) {
                    heroWrapper = document.createElement('div');
                    heroWrapper.className = 'heroSliderWrapper';
                    elem.parentNode.insertBefore(heroWrapper, elem);
                }
                loadHeroSlider(heroWrapper, apiClient, user);
...
export function destroySections(elem) {
    ...
    // Also tear down the hero slider sibling
    const heroWrapper = elem.parentNode?.querySelector('.heroSliderWrapper');
    if (heroWrapper) {
        destroyHeroSlider();
        heroWrapper.remove();
    }
